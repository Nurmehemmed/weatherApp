...
const WeatherCard: FC<WeatherProps> = ({ data }) => {
  ...
  const activeTab =
    dateTabs.find((tab) => tab.value === dateTabqueryValue)?.value ?? 0;

  const formattedDate = convertDateTimeAgo(addDays(new Date(), activeTab));

  return (
    <div className="w-full h-72 md:h-[300px] mt-10 hover:scale-105 transition-all duration-300 w-full  shadow bg-gradient-1 rounded-[32px] px-5 md:px-10 mx-auto  lg:w-[817px] select-none cursor-pointer">
